...
from micro_niche_finder.domain.schemas import CollectionTarget, CollectorRunSummary, GoogleSearchRequest
...
class GoogleCollectorService:
    ...
    def run_once(self, *, session: Session, max_calls: int | None = None) -> CollectorRunSummary:
        ...
        for schedule in due_schedules:
            ...
            try:
                request = GoogleSearchRequest(q=query, num=5)
                response = self.google_search_service.fetch(request)
