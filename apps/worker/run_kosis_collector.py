...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one budgeted KOSIS employee-count collection cycle.")
    parser.add_argument("--max-calls", type=int, default=None, help="Optional hard cap for this run.")
    args = parser.parse_args()

    container = get_container()
    with SessionLocal() as session:
        summary = container.kosis_collector_service.run_once(session=session, max_calls=args.max_calls)
        session.commit()
